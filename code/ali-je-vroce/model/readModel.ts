...
/**
 * Returns the month-day (MM-DD) used to look up historical percentile thresholds
 * for current hotness. average_percentiles is a 365-slot climatology keyed by
...
export function dateForCurrentHotnessPercentiles(
  ...
  const monthDay = formatDateForQuery(new Date(rawTimestamp)).slice(5);
  // No 02-29 row in the climatology → fall back to 02-28.
